...
const mdComponents: Components = {
  ...
  code: ({ className, children }) => {
    ...
    return (
      <code className="bg-emerald-500/15 text-emerald-300 px-1.5 py-0.5 rounded text-sm font-mono">
        {children}
      </code>
    );
  },
...
export const News: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col bg-gradient-dark">
      {/* 头部栏 */}
      <div className="flex-shrink-0 border-b border-border bg-dark-card/50 backdrop-blur-sm px-6 py-4">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-600 flex items-center justify-center text-lg shadow-lg shadow-emerald-500/25">
            N
          </div>
          <div>
            <h1 className="text-xl font-bold text-white">新闻分析</h1>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {market === 'a_share' && (
            <input
              ...
              disabled={isLoading}
              className="px-4 py-2.5 bg-dark text-white rounded-xl border border-border
                focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all
                placeholder:text-gray-600 disabled:opacity-50 w-52"
          ...
          <select
            ...
            className="px-3 py-2.5 bg-dark text-white rounded-xl border border-border
              focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 outline-none transition-all text-sm
              disabled:opacity-50"
          >
          ...
          <button
            ...
            className="px-5 py-2.5 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-xl
              hover:from-emerald-600 hover:to-teal-700 shadow-lg shadow-emerald-500/25
              hover:shadow-emerald-500/40 transition-all duration-300 font-medium
          ...
          {isLoading && (
            <button
              ...
              className="px-4 py-2.5 bg-red-500/15 text-red-400 border border-red-500/20 rounded-xl
                hover:bg-red-500/25 transition-all text-sm font-medium"
            >
              取消
            </button>
          )}
        ...
        {/* 错误提示 */}
        {error && (
          <div className="mt-3 p-3 rounded-xl border bg-red-500/10 border-red-500/20 text-red-400 text-sm">
            {error}
          </div>
        )}
      </div>
      ...
      {/* 情感统计卡片 */}
      {articles.length > 0 && (
        <div className="flex-shrink-0 px-6 py-3 border-b border-border bg-dark-card/30 flex items-center gap-4">
          <span className="text-sm text-gray-400">情感分布:</span>
          ...
          <span className="text-gray-600 text-sm ml-auto">共 {articles.length} 条</span>
        </div>
      )}
      ...
      <div className="flex-1 min-h-0 flex">
          ...
          {articles.length === 0 ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-center py-20">
                <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-emerald-500/20 to-teal-600/20 border border-emerald-500/20
                  flex items-center justify-center text-3xl mx-auto mb-6">
                  N
                </div>
                <h2 className="text-xl font-bold text-white mb-3">新闻分析</h2>
              ...
              {articles.map(article => {
                ...
                return (
                  <div
                    ...
                    className={`px-5 py-4 cursor-pointer transition-all duration-150 hover:bg-dark-light/50 ${
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        <h3 className="text-sm font-medium text-gray-200 leading-6 line-clamp-2">
                          {article.title}
                        </h3>
                        ...
                        <div className="flex items-center gap-3 mt-2 text-xs text-gray-500">
                          ...
                          {sent && sentCfg && (
                            <span className={`px-2 py-0.5 rounded-full text-xs ${sentCfg.bg} ${sentCfg.color} ${sentCfg.border} border`}>
                      ...
                      <button
                        ...
                        disabled={isLoading}
                        className="shrink-0 px-3 py-1.5 text-xs bg-emerald-500/10 text-emerald-400 border border-emerald-500/20
        ...
        <div className="w-[45%] overflow-y-auto">
          {analysisMode === 'none' && selectedArticle ? (
            /* 新闻详情 */
            <div className="p-6">
              <h2 className="text-lg font-bold text-white mb-3 leading-7">{selectedArticle.title}</h2>
              ...
              {selectedArticle.content ? (
                <p className="text-gray-300 leading-7 whitespace-pre-wrap">{selectedArticle.content}</p>
              ) : (
          ...
          ) : (analysisMode === 'single' || analysisMode === 'report') ? (
            /* AI 分析结果 */
            <div className="p-6">
              <div className="flex items-center gap-3 mb-4">
                <div className="w-8 h-8 rounded-lg bg-emerald-500/20 flex items-center justify-center text-xs text-emerald-300">
                  AI
                </div>
              ...
              {analysisMeta && !isAnalyzing && (
                <div className="mt-6 flex items-center gap-4 text-xs text-gray-600 bg-dark-card/50 px-4 py-2 rounded-full w-fit">
            ...
            /* 空状态 */
            <div className="flex items-center justify-center h-full">
              <p className="text-gray-600 text-sm">选择一条新闻查看详情，或点击「AI 分析」进行深度解读</p>
            </div>
          )}
        </div>
      </div>
