"""
基本面选股器核心 — 多因子筛选全市场（财务 + 估值）

从 api/routes/screener.py 下沉而来：route 和 MoneyBill 工具
（screener_tools/recommend_tools）共用这一份，route 只做 HTTP 封装。
"""
...
def resolve_universe(pool_id: Optional[str]) -> Optional[set]:
    """返回限定的 symbol 集合；None 表示全市场"""
    if not pool_id:
        return None
    from data_engine.stock_pools import INDEX_POOLS, fetch_index_constituents, fetch_industry_stocks
    if pool_id in INDEX_POOLS:
        stocks = fetch_index_constituents(INDEX_POOLS[pool_id]["index_code"])
    else:
        stocks = fetch_industry_stocks(pool_id)  # 按行业名
    return {s["symbol"] for s in stocks}


def run_screen(req: ScreenRequest) -> List[dict]:
    """同步执行筛选（在线程池中调用）"""
    session = get_session()
    try:
        ...
        universe = resolve_universe(req.pool_id)
        ...
        for sym in symbols:
            ...
            if req.main_board_only and not is_main_board(sym):
                continue
            # 排除 ST / *ST / 退市风险股（按名称）
            if req.exclude_st:
