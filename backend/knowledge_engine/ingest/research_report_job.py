...
跟 cninfo_job.py 同一套模式：单例 + 线程锁（同一时间只允许一个批次在跑）+
进度文件断点续传（todo = all_symbols - done_set）+ 前端轮询 GET 状态、起停走 POST。
...
class ResearchReportIngestJob:
    ...
    def _run(self) -> None:
        ...
        try:
            ...
            logger.exception(f"研报批量摄入线程异常退出: {e}")
        finally:
