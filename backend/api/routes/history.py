...
async def get_backtest_result(task_id: str):
    ...
    try:
        def _fetch_detail():
            repo = HistoryRepository()
            try:
                return repo.get_backtest_detail(task_id)
            finally:
                repo.close()
        detail = await asyncio.to_thread(_fetch_detail)

        if detail is None:
            raise HTTPException(status_code=404, detail=f"未找到回测结果: {task_id}")

        resp = detail["response"]
        bi = detail["benchmark_inputs"]

        # 获取基准数据（拉曲线的重 IO 留 route，走线程）
        benchmark_data = None
        if bi["start_date"] and bi["end_date"]:
            try:
                def _fetch_benchmark():
                    ...
                    return svc.get_benchmark_curve(
                        ...
                        initial_capital=bi["initial_capital"],
                    )
        ...
        if benchmark_data:
            ...
            resp["metrics"]["excess_return_pct"] = round(
                (resp["metrics"]["total_return_pct"] or 0) - benchmark_data["benchmark_return_pct"], 2
            )
        return resp
