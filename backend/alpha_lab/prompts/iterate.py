...
"""
    ...
    if phase == "explore":
        feedback += """
请基于以上结果，生成一个**改进版** on_bar 信号函数。你可以：
- 尝试完全不同的策略思路（趋势、反转、突破、多因子）
- 调整 buy/sell 的入场与出场条件
- 增加过滤条件减少假信号

注意：固定止损由引擎强制，无需自己写；只用截止当前 bar 的数据（禁未来函数）。
如果过拟合分数偏高，请简化策略（减少参数、用更长周期指标）。

直接输出一个完整的 Python 代码块（只含 on_bar 函数）。
"""
    else:
        feedback += """
