...
**禁止**自己重新定义 BaseStrategy、StrategyContext、Order、OrderType 这些类！它们已由框架提供，直接 import 使用。
...
- `Order(symbol, quantity, order_type, timestamp, price=None)` — 订单对象
  - quantity: 正数=买入, 负数=卖出
  - order_type: `OrderType.MARKET` 或 `OrderType.LIMIT`
- `StrategyContext` 字段:
  - `symbol: str` — 股票代码
  - `current_time: datetime` — 当前日期
  - `current_price: float` — 当前收盘价
  - `data: pd.DataFrame` — 历史 DataFrame（含所有指标）
  - `cash: float` — 可用现金
...
1. **必须 import**：开头必须 `from backtest_engine.strategies.base import BaseStrategy, StrategyContext` 和 `from backtest_engine.portfolio.order import Order, OrderType`
2. **禁止重定义**：不要自己定义 BaseStrategy、StrategyContext、Order、OrderType
3. **参数限制**：__init__ 中的策略参数不超过 5 个
4. **必须止损**：持仓后必须包含止损逻辑（建议 -5% ~ -8%）
5. **禁止硬编码**：不得硬编码特定价格、日期、股票代码
6. **只用可用列**：只使用上面列出的 data 列，不要使用不存在的列
7. **A股规则**：买入数量必须是 100 的倍数
8. **仓位控制**：单次买入不超过可用资金的 95%
9. **数据检查**：使用指标前检查 len(context.data) 是否足够
10. **安全 import**：除了框架 import 外，只能 import pandas, numpy, math, datetime, typing, enum, dataclasses, collections

## 代码格式

- 输出一个完整的 Python 代码块（```python ... ```）
- 必须包含所有 import（框架 import + 标准库）
