...
3. **一个 IP 服务所有国内抓取**（2026-07-10 拍板并实测）。三件事各司其职，别搞混：
   ...
   ⚠️ **IP 消耗的真正驱动是「东财按源 IP 封禁」，不是「代理死了」**（2026-07-10 实测定论）：

   - `requests` 抛的 `ProxyError` **是个误导性标签**。urllib3 只要链路里有代理，就把**目标服务器**在 TLS 握手期的拒绝也包装成 `ProxyError`。实测同一个 IP、同一秒内：百度 200 → 东财 `ProxyError` → 百度 200。代理是活的。
   - 新提取的快代理 IP **不需要预热**，也几乎不会「天生就死」：12/12 首次请求即打通百度。
