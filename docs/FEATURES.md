...
| 决策留痕 | `decision_log.py`、`common/decision_kind.py`、`business_events.py` | AI建议(advisor/cockpit/moneybill)持久化归因 + 业务事件总线→WS推送。表里三类行靠 `entry_kind` 区分（`advice` 才进胜率，回执/操作不进） | 系统内部，各功能调用时自动记录 |
