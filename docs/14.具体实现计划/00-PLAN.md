...
> # 🆕 2026-07-27：项目方向新增一条主线 → `docs/15.策略竞技场/`
>
> Jason 拍板把 AI 从「每笔交易的热路径」挪到「策略迭代的冷路径」：
> **AI 产出的不再是单笔建议，而是策略**；自动引擎执行策略；真实盈亏 feedback 回 AI 调策略。
>
> **对本文件的三个影响**（开任何一张卡前先看）：
>
> 1. ✅ **下面 §1 的第 1 条约束（每笔交易人工审核，永不放开）已被 Jason 授权变更**，
>    §1 正文已于 2026-07-27 由 `S0-定位变更.md` 改完，与 doc15 §1 一致。
> 2. **P0-4 批次3 已迁出**：查询侧可见性 → doc15 `S0`（✅ 2026-07-27 完工）；
>    写入侧（双重留痕收口/归因）→ doc15 `S4`。
> 3. **三张卡与 doc15 重叠，开工前必须先对齐**（否则重复建设）：
>    **P1-4**（失效条件监控 ≈ doc15 S3 的「卫冕者变弱检测」）、
>    **投资组合模块**（资金分配 ≈ S3 的组合权重；⚠️ 那张卡里记的
>    「取消单股上限**与**留 20% 现金」是**错的** —— 正确口径是
>    **单股上限取消、20% 现金必须保留**，以 §1 为准）、
>    **P1-6 B 段**（绑投资组合模块 → 排在 S3 之后）。

---

## 1. 项目定位（唯一权威表述；🔄 2026-07-27 由 Jason 授权修订，原「不可改」标记已去掉）

**服务 Jason 一个人的实盘交易工具（真金白银，非模拟）。AI 是研究助手，必须明确给出「怎么买、怎么卖」，不许和稀泥。**

> 🔄 **2026-07-27 变更（授权人：Jason）**。**旧口径**（2026-07-17 拍板）是：
> 「**每笔交易人工审核，永不放开**——即使将来接通券商 API 自动下单」。
> 它**已被替换**（不是被保留、也不是「审核点上移所以约束没破」——那是美化）。
> 变更起因与全部裁决：`docs/15.策略竞技场/00-PLAN.md` §1；执行卡：`S0-定位变更.md`。

三条约束：

1. **策略变更必须人工审核**——AI 只能**提议**改策略，不能自行 arm 一条策略上线。
   单笔交易金额**无上限**，约束交易的是硬风控而不是单笔天花板：
   单股/单笔上限 ❌ 已取消；**总持仓 ≤ 80%、必须保留 20% 现金 🔒 保留，不许取消**；
   单日亏损 ≤ 3%、每笔必须止损、连亏 3 次暂停 🔒 全部保留。
   ⚠️ **别记错这个组合**：放开的是「单笔能下多大」，没放开的是「总共能压多少」。
   代码承载物：`agents/confirm_gate.py` + `requires_confirmation=True` **仍不可移除**——
   它管的对象从「每笔交易」变成「每次策略变更」，东西还在；
   现金底线由 `trading_engine/risk/adapter.py::get_max_total_position_pct()` 独家提供，
   门禁 `tests/test_risk_total_position_floor.py`。
2. **AI 是研究助手，不是决策者**——但它要预测涨跌、要直接说 BUY/SELL/HOLD；
   策略切不切换由**规则**判定，不由 AI 拍板。
