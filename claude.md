...
**风控规则（硬编码，不可绕过）**（🔄 2026-07-27 Jason 更新）：
- ~~单股最大仓位 ≤ 总资金 20%~~ → **已取消：单笔交易金额无上限**
- 🔒 **总持仓不超过 80%，必须保留 20% 现金** ← **这一条保留，不许取消**
- 单日最大亏损 ≤ 总资金 3%，触发自动停止交易
- 每笔交易必须设置止损（默认 -5%）
- 连续亏损 3 次后暂停交易 1 天

> ⚠️ **别记错这两条的组合**：单笔**无上限**，但总仓位**必须留 20% 现金**。
> 放开的是「单笔能下多大」，没放开的是「总共能压多少」。
...
- 新策略上实盘前必须先通过模拟交易验证
- 🔄 **审核点已变更（2026-07-27 Jason 授权）**。旧口径「所有交易操作需要二次确认，每笔交易仍须人工审核，永久硬约束」（2026-07-17 拍板）**已被替换**为下面三条：
  1. **策略变更必须人工审核** —— AI 只能**提议**改策略，不能自行 arm 一条策略上线
  2. **单笔交易金额无上限，但受硬风控约束**（尤其**必须保留 20% 现金**，见「风控规则」）
  3. **AI 是研究助手，不是决策者** —— 策略切不切换由**规则**判定，不由 AI 拍板

  ⚠️ **诚实表述**：这**就是放开了逐笔人工审核**，旧约束是被**替换**而非保留。
  ⛔ 但 `agents/confirm_gate.py` + `requires_confirmation=True` **仍不可移除** —— 它改管「策略 arm」，东西还在，只是管的对象从「每笔交易」变成「每次策略变更」。
  📖 完整背景与全部裁决见 `docs/15.策略竞技场/00-PLAN.md`
- 风控模块不可被策略代码覆盖
...
1. **风险警告**：这是个生产级交易系统，**现在就是实盘交易——真金白银**（2026-07-17 更正，原写「以模拟交易为主」已过期）。
   **定位**：服务 Jason 一人的实盘工具；**AI 是研究助手，必须明确给出「怎么买、怎么卖」，不许和稀泥**。
   🔄 **2026-07-27 起，AI 的产出物从「单笔建议」升级为「策略」**：AI 提议策略变更 → Jason 审核 → 自动引擎执行。
   原「每笔交易仍须人工审核——这一条永不放开」**已变更**，新口径见上方「安全规范」与 `docs/15.策略竞技场/00-PLAN.md` §1。
